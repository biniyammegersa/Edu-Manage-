...
export function Notifications() {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <Card className="absolute right-0 mt-2 w-96 z-50 bg-gray-900 shadow-2xl">
          ...
          <CardContent>
            <ScrollArea className="h-[400px] pr-4">
              {notifications.map((notification) => (
                <Card
                  key={notification.id}
                  className="mb-4 last:mb-0 border shadow-sm bg-gray-700/60 hover:bg-gray-700/30 transition duration-200 ease-in-out"
                >
